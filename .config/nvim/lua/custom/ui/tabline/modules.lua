local fn = vim.fn
local api = vim.api
local buf_is_valid = require("custom.ui.tabline.utils").buf_is_valid
...
local new_hl = function(group1, group2)
  ...
  api.nvim_set_hl(0, new_hlgroup, { fg = fg, bg = bg })
  return new_hlgroup
end
...
local generate_buf_str = function(v, bufnr)
  ...
  local hlgroup = vim.fn.getbufinfo(v)[1].hidden == 1 and "TabLineBufHidden" or "TabLineBufActive"
...
M.padding_ft = function(ft)
  ...
  return hl_str("TabLineFill", string.rep(" ", get_ft_win_width(ft) + 1))
end
...
M.tablist = function()
  ...
  for i, v in ipairs(tablist) do
    if v == tabnr then
      result = result .. hl_str("TabLineCurrentTab", string.format(" %-3d", i))
    else
      result = result .. hl_str("TabLineOtherTab", string.format(" %-3d", i))
    end
  end
  return result
end
